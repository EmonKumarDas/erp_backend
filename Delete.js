...
const deleteCompany = (app, CompanyCollection, ObjectId, verifyJWT) => {
    app.delete('/deleteCompany/:id', verifyJWT, async (req, res) => {
        const id = req.params.id;
        const filter = { _id: new ObjectId(id) };
        const result = await CompanyCollection.deleteOne(filter);
        res.send(result);
    })
}

module.exports = { deleteProduct, deleteshop, deleteCompany };